...
const STATIC_CACHE = 'dlo-kupwara-static-v10';
...
self.addEventListener('fetch', event => {
  ...
  // App-shell files (JS/CSS) always try the network first while this site is
  // under active development — cache-first here is exactly what causes the
  // "I updated the file but the browser won't show it" class of bug, since
  // an old service worker keeps serving its own stale cached copy of
  // common.js/menu.css until its update cycle catches up, however many
  // times the file itself or the cache-version string changes. Falling back
  // to cache only on an actual network failure keeps real offline support.
  event.respondWith(
    fetch(req).then(res => {
