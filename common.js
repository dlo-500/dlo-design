...
(function (global) {
  ...
  function translationLookup(key, page) {
    ...
    const p = page || translationPageKey();
    const pageDict = global.DLO_TRANSLATIONS.pages && global.DLO_TRANSLATIONS.pages[p];
    const commonDict = global.DLO_TRANSLATIONS.common || {};
    const candidates = [];
    ...
    for (const c of candidates) {
      if (pageDict && pageDict[c]) return pageDict[c];
      if (commonDict[c]) return commonDict[c];
    }
    return null;
  }
  ...
  function translateKey(key, lang, page) {
    ...
    if (entry && entry[lang]) {
      // Preserve punctuation present in the original source when the dictionary
      // entry is keyed without that final punctuation.
      const source = String(key);
      const suffix = (source.match(/[.?!:;]+$/) || [''])[0];
      const translated = entry[lang];
      if (suffix && !/[.?!:;]+$/.test(translated)) return translated + suffix;
      return translated;
    }
    if (global.DLO_TRANSLATE) return global.DLO_TRANSLATE(String(key).trim(), lang, page || translationPageKey());
    return key;
  }

  function translateTextNode(node, lang, page) {
    if (!node || !node.nodeValue || !node.nodeValue.trim()) return;
    const original = node.__dloOriginalText != null ? node.__dloOriginalText : node.nodeValue;
    node.__dloOriginalText = original;
  ...
  function applyLanguage(lang, options) {
    ...
    // Restore every original text node before applying another language. This
    // is important for headings that contain <em>, <strong>, icons, etc.
    const restoreWalker = document.createTreeWalker(rootForTranslation(), NodeFilter.SHOW_TEXT);
    let restoreNode;
    while ((restoreNode = restoreWalker.nextNode())) {
      if (restoreNode.__dloOriginalText != null) restoreNode.nodeValue = restoreNode.__dloOriginalText;
    }
    ...
    if (lang !== 'en' && _translationReady) {
      ...
      nodes.forEach(n => {
        ...
        const source = n.__dloOriginalText != null ? n.__dloOriginalText : n.nodeValue;
        if (!source || !source.trim()) return;
        n.__dloOriginalText = source;
