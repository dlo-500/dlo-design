...
(function (global) {
  ...
  function calculateStats(rows) {
    ...
    rows.forEach(r => {
      ...
      if (exparteVal.includes('ex-parte') || exparteVal.includes('exparte') ||
  ...
  function applyLanguage(lang, options) {
    ...
    if (lang !== 'en') {
      ...
      root.querySelectorAll('[title],[placeholder],[aria-label],[alt],input[value],button[value]').forEach(el => {
        ['title','placeholder','aria-label','alt','value'].forEach(name => {
          if (!el.hasAttribute(name)) return;
          const source = el.getAttribute(name);
          if (!source || !source.trim()) return;
          const translated = translateKey(source.trim(), lang, page);
          if (translated !== source.trim()) {
            el.setAttribute('data-dlo-original-attr', source);
            el.setAttribute('data-dlo-original-attr-name', name);
    ...
    function buildPopoverButton(opts) {
      ...
      opts.items.forEach(item => {
        ...
        optBtn.addEventListener('click', (e) => {
          // Stop the click from bubbling to the toolbar or document.
          // Without this, touch browsers fire a synthetic second tap that lands
          // on whatever is rendered beneath the now-closed popover (e.g. searchBtn).
          e.stopPropagation();
          opts.onSelect(item);
          closePop();
        });
